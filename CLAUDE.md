...
`BASE_COLORS` is the source palette (indices 1–20, shared with `PIECES`); it is never drawn directly. The `SKINS` registry (retro/neon/pastel/pixel/matrix/blueprint/gameboy/glass) derives two palettes per skin from `BASE_COLORS` once at load (`initSkinPalettes` → `SKINS[key].palettes.{dark,light}`), and `colorFor(index)` — which reads `SKINS[skin].palettes[theme][index]` — is the only accessor `drawBlock`, `renderEffects`, and `drawStatusBanners` should use to read a color. The module-level `theme` variable ("dark"/"light") is set by `applyTheme` alongside its `dataset.theme` write, so switching theme or skin always redraws both the board and the `#next-canvas` preview. Each skin declares a `block` style (`flat`/`glow`/`rounded`/`pixel`/`glyph`/`outline`/`lcd`/`glass`) consumed by `drawBlock`'s `switch`, plus optional per-theme `dark`/`light` overrides for `derive` (the color-transform function), `canvasBg`, `grid`, and `icon` — read through `skinProp(name)`, which prefers the active theme's override and falls back to the skin's base value. Switching skins (`applySkin`) sets `document.documentElement.dataset.skin`, letting `style.css` `[data-skin="..."]` / `[data-theme="light"][data-skin="..."]` blocks restyle the panel/overlay on top of the light/dark theme tokens.
